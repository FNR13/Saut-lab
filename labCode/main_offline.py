...
def main():
    ...
    N_PARTICLES = 150
    particles_odometry_uncertainty = (0.011, 0.08)  # (speed, angular rate)
    landmarks_initial_uncertainty = 0.5
    Q_cov = np.diag([0.3, np.pi/8])  # Measurement noise covariance for range and bearing
    ...
    # Plot best path
    ax1.plot(paths[best_path, :, 0], -paths[best_path, :, 1], label='Most probable path', color='blue', linewidth=2)
