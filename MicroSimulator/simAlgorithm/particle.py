...
from simAlgorithm.ekf import ExtendedKalmanFilter, compute_jacobians
# from ekf import ExtendedKalmanFilter, compute_jacobians # For debugging
...
class Particle:
    ...
    def landmark_update(self, marker_id, z):
        ...
        if marker_id in self.landmarks_id:
            ...
            self.landmarks_position.append(landmark_Position.reshape(2, 1))
            self.landmarks_position_covariance.append(landmark_position_covariance.reshape(2, 2))
            self.landmarks_observation_count.append(0)
            self.landmarks_EKF.append(newKalmanFilter)
            

    def compute_weight(self, marker_idx, z, pose):
            z = z.reshape(2, 1)

            xf = self.landmarks_EKF[marker_idx].landmark_position
            Pf = self.landmarks_EKF[marker_idx].landmark_covariances
